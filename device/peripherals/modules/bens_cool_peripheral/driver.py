...
from device.peripherals.modules.bens_cool_peripheral import simulator, exceptions
...
class BCPDriver:
    """Driver for bcp temperature and humidity sensor."""
    ...
    def __init__(
        ...
        if simulate:
            ...
            Simulator = simulator.BCPSimulator
        else:
            Simulator = None
    ...
    def read_temperature(self, retry: bool = True) -> Optional[float]:
        ...
        # BCP is 12-bit so max temperature processing time is 22ms
        time.sleep(0.22)
    ...
    def read_humidity(self, retry: bool = True) -> Optional[float]:
        ...
        # Wait for sensor to process, see datasheet Table 7
        # BCP is 12-bit so max humidity processing time is 29ms
        time.sleep(0.29)
